...
## Post-Phase-9 adjustments — setup.sh template files (real, not a heredoc)
User asked for a maintainable `dots-local` template, using chromaden's
current `dots-local` as the baseline shape - considered a Nix-package-
based generator vs. plain template files copied/filled in directly.
Chose plain template files (`templates/dots-local/{flake.nix,
appimages.nix,gitignore}`) - real, standalone, syntactically valid Nix
files using `@@TOKEN@@` placeholders, `setup.sh` now just `cp`s +
`sed -i`s them rather than interpolating bash variables into an inline
heredoc (which needed things like `\${march}` double-escaping before).
Rejected the Nix-package route specifically because `setup.sh` runs
*before* any working `dots-local` exists - adding a dependency on
building something from `dots`'s own flake just to generate the very
first `dots-local` file is unnecessary complexity for "copy a file and
substitute a few values."

`templates/dots-local/flake.nix` mirrors chromaden's real shape/field
order; required identity fields are placeholders, everything optional
(gpu/compositor/isWsl/machine.*/extraModules/tune.flags/sync) stays
commented-out examples - doesn't need to be exhaustive since
`dots-local-options` (previous entry) now generates the full reference
live from the schema.

Also fixed two pre-existing, unrelated doc bugs found while in this
area: SYNC.md's "Initial setup on new machine" workflow told the reader
to run `setup.sh` from `~/dots-local` with no argument (wrong directory,
missing the required profile arg); its "File Relationships" tree also
mislabeled `setup.sh` as living inside `dots-local/` rather than `dots/`.

Validated: fresh-setup regression test (sandboxed `$HOME`, ran the
identity-generation half of `setup.sh`, then `nix eval`/`nix build` the
result) - template-generated `dots-local` evaluates and builds cleanly
end-to-end; chromaden's own real `dots-local` re-verified unaffected
(zero new derivations - it's hand-edited already, not regenerated by
this change).

## Post-Phase-9 adjustments — wrap-up audit (duplicate packages, dead options, doc gaps)
User asked for a final "anything unhandled/lingering/further
consolidation" pass. Fixed: `git-tools.nix` rewritten onto `mkAppSet`
(lazygit/gh now alien-aware, matching every other suite); removed
zellij/lazygit/bash/delta duplicate `home.packages` entries (same class
as the round-5 direnv/lsd/zoxide/fzf/bat fixes); documented the
accepted wezterm package-duplication tradeoff; removed dead
`piDataDir` in ai-apps.nix; wired up butterfish's previously-inert
`shell` option; added README feature-table rows for
butterfish/llama-cpp/nix/quarkdown/sd-switch/wsl-shell-integration;
resolved/pruned two stale `open-questions.md` entries. Considered, then
reverted, a change to `nix-tools.nix`'s `lib.mkIf`-in-a-list pattern
after confirming empirically it isn't actually a bug (see
`decisions.md` and `learnings.md`'s "listOf v2 merge" entry for the
full story). Validated via full build + byte-diff of
