      ...
      _clip_process() {
        ...
        # 1. Strip ANSI
        [ "$strip_ansi" = "true" ] && pipeline="$pipeline | $SED_BIN -r \"s/\x1B\[([0-9]{1,3}(;[0-9]{1,3})*)?[mGK]//g\""
        ...
        if [ "$no_trim" = "false" ]; then
           ...
           pipeline="$pipeline | $PERL_BIN -pe 'chomp if eof'"
        fi
      ...
      _clip_logic() {
        ...
        _do_copy_buffered() {
          ...
          if [ -s "$tmp_raw" ]; then 
            _clip_process "$s" "$r" "$T" < "$tmp_raw" | "${COPY_CMD[@]}"
          fi
          rm -f "$tmp_raw"
        }
      ...
      clipfile() {
        ...
        [ -f "$1" ] && _clip_process "$s" "$r" "$T" < "$1" | "${COPY_CMD[@]}"
      }
